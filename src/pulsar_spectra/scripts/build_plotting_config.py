...
def parse_args():
    parser = argparse.ArgumentParser(
        usage="Usage: %(prog)s [options]", description="Create a plotting configuration file."
    )
    ...
    marker_generation.add_argument(
        "--marker_file_savename",
        action="store",
        type=str,
        dest="marker_file_savename",
        default=None,
    ...
    if not is_valid_colour(args.model_colour):
        parser.error("invalid model colour")

    if not is_valid_colour(args.model_error_colour):
        parser.error("invalid model error colour")
...
def generate_marker_set(
    ...
    if palette_name == "TAB":
        ...
        print("Error: palette name not valid. Defaulting to IBM.")
        palette = PALETTE_IBM
    ...
    while num_left_to_generate > 0:
        ...
        for i in range(generate_on_current_loop):
            if uniform_size:
                marker = [
                    f"{colours[i][1]} {markers[i][2]}",
                    colours[i][0],
                    markers[i][0],
                    round(marker_size * markers[i][1], 2),
                ]
            else:
    ...
    if plot_preview:
        plt.savefig("marker_preview.png", dpi=300, bbox_inches="tight")
        plt.clf()
